...
bio_df = min_yr:max_yr %>%
  ...
  map_df(.id = 'Year',
         .f = function(yr) {
           ...
           data_df = read_excel(paste0('analysis/data/raw_data/WDFW/PIT_Cleaned_', yr, ' SPCH Data.xlsx'),
                           sheet = 'Bio Data')
...
#               path = paste0('analysis/data/raw_data/tag_lists/Tumwater_Chnk_Tags_', yr, '.txt'),
#               delim = '\n',
#               col_names = F)
# }
...
write_delim(tag_list[[as.character(max_yr)]],
            path = paste0('analysis/data/raw_data/tag_lists/Tumwater_Chnk_Tags_', max_yr, '.txt'),
            delim = '\n',
            col_names = F)
